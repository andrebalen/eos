...
CMD bash -c ". /tmp/.helpers && $PRE_COMMANDS && \
    ...
    if ! ${TRAVIS:-false}; then cd .. && $(pkill mondogd || :) && tar -pczf build.tar.gz build && buildkite-agent artifact upload build.tar.gz --agent-access-token $BUILDKITE_AGENT_ACCESS_TOKEN; fi"
